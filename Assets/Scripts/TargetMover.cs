...
public class TargetMover : MonoBehaviour
{
    ...
    protected bool atTarget = false;  // This property is set to "true" whenever the object has already found the target.

    private bool isMoving = false;

    void Update()
    {
        if (!atTarget && !isMoving)
        {
            isMoving = true;
    ...
    {
        Debug.Log("SetTarget(" + newTarget + "," + targetInWorld + ")");
        if (targetInWorld != newTarget)
        {
            Debug.Log("targetInWorld != newTarget");
